...
This is going to be a pretty cool hook. It will be used pretty much the same way as `useState`, but with a key and value passed into it - ie `const [name, setName] = useLocalStorage('name', 'Dustin')`. You can use `setName`4 to update the value of `name` on localStorage! Pretty cool, huh? Let's get to it!
